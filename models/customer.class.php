...
class Customer implements JsonSerializable{
	...
	public function set($id,$name,$mobile,$email,$password,$country_id,$street_address,$city,$postcode,$apartment){
		$this->id=$id;
		$this->name=$name;
		$this->mobile=$mobile;
		$this->email=$email;
	...
	public static function take($username,$password){
		global $db,$tx;
		$result=$db->query("select id,name,mobile,email,password,country_id,street_address,city,postcode,apartment from {$tx}customers where (name='$username'or id='$username'or email='$username') and password='$password' ");
	...
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}customers set name='$this->name',mobile='$this->mobile',email='$this->email',password='$this->password',country_id='$this->country_id',street_address='$this->street_address',city='$this->city',postcode='$this->postcode',apartment='$this->apartment' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,mobile,email,password,country_id,street_address,city,postcode,apartment from {$tx}customers");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,mobile,email,password,country_id,street_address,city,postcode,apartment from {$tx}customers $criteria limit $top,$perpage");
		$data=[];
		while($customer=$result->fetch_object()){
			$data[]=$customer;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,mobile,email,password,country_id,street_address,city,postcode,apartment from {$tx}customers where id='$id'");
		$customer=$result->fetch_object();
			return $customer;
	}
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($customer=$result->fetch_object()){
			...
			$html.="<tr><td>$customer->id</td><td>$customer->name</td><td>$customer->mobile</td><td>$customer->email</td><td>$customer->password</td><td>$customer->country_id</td><td>$customer->street_address</td><td>$customer->city</td><td>$customer->postcode</td><td>$customer->apartment</td> $action_buttons</tr>";
		}
		$html.="</table>";
