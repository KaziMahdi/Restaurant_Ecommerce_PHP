...
class Menu implements JsonSerializable{
	...
	public static function filter_by_category($category_id){
		global $db,$tx;
		$result=$db->query("select id,name,offer_price,regular_price,description,photo,category_id,offer_discount,uom_id,barcode,created_at,updated_at from {$tx}menus where category_id='$category_id' ");
		$data=[];
		while($menu=$result->fetch_object()){
			$data[]=$menu;
		}
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Offer Price</th><th>Regular Price</th><th>Description</th><th>Photo</th><th>Category Id</th><th>Offer Discount</th><th>Uom Id</th><th>Barcode</th><th>Created At</th><th>Updated At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Offer Price</th><th>Regular Price</th><th>Description</th><th>Photo</th><th>Category Id</th><th>Offer Discount</th><th>Uom Id</th><th>Barcode</th><th>Created At</th><th>Updated At</th></tr>";
		}
		while($menu=$result->fetch_object()){
			$action_buttons = "";
			if($action){
				$action_buttons = "<td><div class='clearfix' style='display:flex;'>";
				$action_buttons.= action_button(["id"=>$menu->id, "name"=>"Details", "value"=>"Details", "class"=>"info", "url"=>"details-menu"]);
			...
			$html.="<tr><td>$menu->id</td><td>$menu->name</td><td>$menu->offer_price</td><td>$menu->regular_price</td><td>$menu->description</td><td><img src=\"img/$menu->photo\" width=\"100\" /></td><td>$menu->category_id</td><td>$menu->offer_discount</td><td>$menu->uom_id</td><td>$menu->barcode</td><td>$menu->created_at</td><td>$menu->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
