...
class SiteMenu implements JsonSerializable{
	...
	public function set($id,$name,$link,$has_child){
		$this->id=$id;
		$this->name=$name;
		$this->link=$link;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}site_menus(name,link,has_child)values('$this->name','$this->link','$this->has_child')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}site_menus set name='$this->name',link='$this->link',has_child='$this->has_child' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,link,has_child from {$tx}site_menus");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,link,has_child from {$tx}site_menus $criteria limit $top,$perpage");
		$data=[];
		while($sitemenu=$result->fetch_object()){
			$data[]=$sitemenu;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,link,has_child from {$tx}site_menus where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Link</th><th>Has Child</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Link</th><th>Has Child</th></tr>";
		}
		while($sitemenu=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$sitemenu->id</td><td>$sitemenu->name</td><td>$sitemenu->link</td><td>$sitemenu->has_child</td> $action_buttons</tr>";
		}
		$html.="</table>";
